...
* Further information can be found in README.txt                               *
*                                                                              *
* PLEASE NOTE THAT THE ENTIRE CODE TAKES ABOUT TWO WEEKS TO EXECUTE.           *
* Plan Accordingly.                                                            *
*------------------------------------------------------------------------------*

*local folders
global MAIN_FOL "H:/DnD_Replication"
...
/*--------------------*/ 
	...
	/* SUMMARY STATISTICS */
		cap log close
		cap log using "${RES}/XLS/TABLE_2_${VERSION}_sumstat.log" , replace
		do "${DOFILES}/SUMSTAT.do"
		cap log close
		cap log using "${MAIN_FOL}/log_STATNL" , append
	...
	/* PREAMBLE */	
		...
		*global macros
			global NKIDS 			= `nkids'
			global SAMPLE   		= 10 //sampling criterion
			global MC_ONOFF 	 	 `MC_onoff'
			global MC_SAMPLE 		= `MC_sample'
			global MC_ITERATIONS	= `MC_iterations'
			global ROBUST_EXT 		= 0 // extensive robustness checks switch
	...
	* ESTIMATES FOR TABLE A1, Column 1
		cloglog event ib(first).AGE_K_Y1  daughter1#ib(first).AGE_K_Y1 NOCHX , eform robust
			EST_SPCOUNT count if e(sample)==1 
			estimates save "${EST}/reg_${VERSION}_main_1'", replace
			drop NOCHX
	 
	/* -1b- principal multivariate model ------------------------------------ */
	* ESTIMATES FOR FIGURE 1a & 1b, TABLE A1, Column 2
	...
	* ESTIMATES FOR FIGURE 2a-2d, TABLE A1, Column 3
	...
	* ESTIMATES FOR TABLES 3, A2 & B1
		...
		** estimate subgroup models and save results
			...
			local HET EU
			foreach i in 0 1 {
				merge m:1 CID using "${DATA}/DIV_GEND_ADD_INFO", keep(match)
				for num 1/2: gen byte EUX = LANDX ==5002 | LANDX ==5003 | LANDX ==5009 | LANDX ==5010  | LANDX ==5015 | LANDX ==5017 | LANDX ==5032 | LANDX ==5039 | LANDX ==5056 | LANDX ==6002 | LANDX ==6003 | LANDX ==6007 | LANDX ==6018 | LANDX ==6027 | LANDX ==6029 | LANDX ==6030 | LANDX ==6037 | LANDX ==6039 | LANDX ==6053| LANDX ==6055 | LANDX ==6066 | LANDX ==7024| LANDX ==7028| LANDX ==7044| LANDX ==7047| LANDX ==7048| LANDX ==7050| LANDX ==7064| LANDX ==7065| LANDX ==7066| LANDX ==7085 | LANDX ==9089
				 
				drop if GEN1 ==0 & GEN2 ==0
				keep if `HET'1==`i' & `HET'2==`i'
				cap cloglog event2 ${HETVARLIST} ,eform
				EST_SPCOUNT_MC count, ${MC_ONOFF} it(${MC_ITERATIONS}) sample(${MC_SAMPLE}) auxkeep
				drop _all
				cap estimates save "${EST}/reg_${VERSION}_het_`HET'_`i'", replace	
				restore, preserve
			...
			*maternal siblings
			cap preserve 
				...
				rename rinpersoon2 rinpersoon
				merge m:1 rinpersoon   using  "${DATA}/GENDKIDRIN12MAX.dta" , keep(match) nogen
...
/*------------------------------ ROBUSTNESS - EXTENDED -----------------------*/
 ...
 if ${ROBUST_EXT} == 1 {
	...
	*Specification charts - require the dataset to be loaded 
		...
		graph export "${RES}/XLS/FIGURE_3_${VERSION}_specs.png", as(png)
	
	*robustness - cohabiting couples
	*(TABLE B1c, Last column)
		do "${DOFILES}/ROBUST_COH.do"
	...
	*(TABLE B2, Col. 3)
		use `baseline', clear
		keep if t_BD1 >= monthly("1995m01","YM") & TM_Y<37
		cloglog event ib(first).AGE_K_Y1 daughter1#ib(first).AGE_K_Y1 ${DURSPEC} ${BG_VARS} , eform robust
		EST_SPCOUNT_MC count, it(${MC_ITERATIONS}) sample(${MC_SAMPLE}) ext
	...
	*(TABLE B2, Col. 4 & 5)
		...
		cloglog event ib(first).AGE_K_Y1 daughter1#ib(first).AGE_K_Y1 ${DURSPEC} ${BG_VARS} PREMAR if i_nobro ==1, eform robust
		estimates save "${EST}/reg_${VERSION}_robu_4", replace
	...
	*(TABLE B2, Col. 6)
		...
		*retrieve incomes and employment prior to the year of outcome
		...
		cloglog event ib(first).AGE_K_Y1 daughter1#ib(first).AGE_K_Y1 ${DURSPEC} ${BG_VARS} PREMAR emp* INC* , eform  robust
		EST_SPCOUNT_MC count, it(${MC_ITERATIONS}) sample(${MC_SAMPLE}) ext
...
*------------------------------------------------------------------------------*
	...
	/* OUTPUT FOR FIGURES 1 & 2, TABLE A1 - MAIN RESULTS */
		...
		cap erase XLS/TABLE_B2_${VERSION}_robu.txt	
		cap gen iKIDS = _n<(_N/2)
	...
	/* OUTPUT FOR TABLE B2 - ROBUSTNESS */
			...
			local outreglist i.AGE_K_Y1 daughter1#i.AGE_K_Y1 i.TM_Y  ${BG_VARS} PREMAR iKIDS iA* iD*
	...
	/* OUTPUT FOR TABLE B1 - COHABITATION */
		...
		outreg2 using XLS/TABLE_B2_${VERSION}_het.xls, keep(`outreglist' ) dec(3) noparen excel label ctitle("`HET'`i'")   stats(coef se) eform addtext(spells,`cnt', ll,`ll',Cohort FE, YES)
